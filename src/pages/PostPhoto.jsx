...
function PostPhoto() {
  ...
  const [editReply, setEditReply] = useState({ parentId: null, id: null, text: '' });
  ...
  useEffect(() => {
    ...
    const fetchedComments = [
      { id: 1, author: '박**', text: '웅 난 그냥 다 말해', time: '45분 전', profilePic: 'https://via.placeholder.com/40', replies: [] },
      { id: 2, author: '강**', text: '걍 굳이... 그런 말은 안하는게 나음', time: '07 / 05', profilePic: 'https://via.placeholder.com/40', replies: [] },
      { id: 3, author: '최**', text: '담배피는게 흠임?', time: '07 / 06', profilePic: 'https://via.placeholder.com/40', replies: [] },
      { id: 4, author: '김**', text: '말하고 싶으면 하는 거고 아님 마는거지', time: '07 / 06', profilePic: 'https://via.placeholder.com/40', replies: [] },
      { id: 5, author: '김**', text: 'ㅋㅋ', time: '07 / 07', profilePic: 'https://via.placeholder.com/40', replies: [] },
      { id: 6, author: '류**', text: '강 하셈', time: '07 / 07', profilePic: 'https://via.placeholder.com/40', replies: [] },
    ];
  ...
  return (
    <div className="App flex flex-col h-screen max-w-lg mx-auto">
      ...
      <header className="flex items-center justify-between p-4 border-b">
        <FaArrowLeft className="text-xl cursor-pointer" onClick={() => navigate('/community')} />
      ...
      <main className="flex-1 p-4 overflow-auto">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-600">{postData.date} | {postData.author}</p>
                    ...
                    <ul className="mt-2 space-y-2 pl-4 border-l border-gray-300">
                      {comment.replies.map((reply) => (
                        <li key={reply.id} className="flex items-start">
                          <img src={reply.profilePic} alt="Profile" className="w-8 h-8 rounded-full mr-2" />
                          <div className="flex-1">
                            {editReply.parentId === comment.id && editReply.id === reply.id ? (
                              <div className="flex items-start">
                                <textarea
                                  value={editReply.text}
                                  onChange={(e) => handleEditReplyChange(e, comment.id)}
                                  className="flex-1 p-2 border rounded-md focus:outline-none"
                                />
                                <button
